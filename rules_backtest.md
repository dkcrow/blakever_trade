...
- **排名表格**：一行一只ETF，8列（排名/名称/代码/综合分/短期/长期/价格/日期/变动）
- **变动列**：显示 `↑2`、`↓1`、`→0`（对比上次排名），实时/CSV标签跟在后面
- **高亮**：第1名黄色背景（`rank-1`），当前持仓黄色背景（`highlight`）
- **字体**：表头12px加粗，行12px，说明区11px灰色，页脚10px
- **排名变动字体**：11px，和说明区一致
- **前三名**：不用emoji🥇🥈🥉（避免GBK编码问题），用CSS背景色区分

**更新时间**：2026-05-25 11:40 用户确认
