...
# -*- coding: utf-8 -*-
"""
三马七星策略 - 美股个股版本 V2（修复版）
=======================================
修复问题：
1. 盈利保护改为追踪自买入以来的最高点（而非20日高点）
2. 移除SNDK（数据不足）
3. 增加调仓频率检查

成分股：NVDA, AMD, MU, AVGO, TSLA, AAPL, GOOG, AMZN, KO, NEM, XOM, AEP, JPM, GS, BRK-B
"""
...
# Backtrader 策略（修复版）
# ================================================================
class QixingSanmaV2(bt.Strategy):
    params = (
    ...
    def __init__(self):
        self.stock_data = {}
        self.holdings = {}  # {name: {'entry_price': x, 'peak_price': y}}
        self.trade_log = []
    ...
    def next(self):
        ...
        for name, pos in list(self.holdings.items()):
            ...
            if peak_price > entry_price and price / peak_price - 1 <= -self.p.profit_protect_pct:
                ...
                print(f'  [PROTECT] Profit protect {name}: {price:.2f} (peak:{peak_price:.2f}, pnl:{pnl:.1f}%)')
                to_remove.append(name)
                continue
        ...
        if top_score > 0:
            ...
            size = int(self.broker.getcash() * 0.95 / d.close[0])
            if size > 0:
                self.buy(data=d, size=size)
                self.holdings[top_stock] = {
                    'entry_price': d.close[0],
                    'peak_price': d.close[0]
                }
                self.trade_log.append({
                    'date': self.datas[0].datetime.date(0),
                    'action': 'BUY',
...
def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    ...
    print(f'  盈利保护: {PROFIT_PROTECT_PCT*100:.0f}%回撤（从峰值）')
    print(f'\n加载数据...')
    ...
    cerebro.addstrategy(QixingSanmaV2)
