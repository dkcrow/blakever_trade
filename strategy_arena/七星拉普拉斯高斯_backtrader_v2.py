...
# -*- coding: utf-8 -*-
"""
七星拉普拉斯高斯策略 - Backtrader版本 v2
===================================================
克隆自聚宽文章：https://www.joinquant.com/post/70329
原作者：king088

改写：本地Backtrader回测版，用yfinance下载A股ETF数据
"""
...
def load_etf_data(etf_code):
    ...
    # 下载
    df = fetch_etf_data_yfinance(etf_code)
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def _calc_momentum(self, d, etf):
        ...
        try:
            ...
            closes = [d.close[i] for i in range(-min(lookback, len(d)), 0)]
    ...
    def _check_range_bound(self):
        ...
        closes = [d.close[i] for i in range(-(max(MA_PERIOD, LOOKBACK_HIGH_LOW_DAYS) + 5), 0)]
        highs = [d.high[i] for i in range(-(max(MA_PERIOD, LOOKBACK_HIGH_LOW_DAYS) + 5), 0)]
        lows = [d.low[i] for i in range(-(max(MA_PERIOD, LOOKBACK_HIGH_LOW_DAYS) + 5), 0)]
