...
# -*- coding: utf-8 -*-
"""
拉普拉斯策略 - 终极版（2026-05-24）
核心逻辑：
1. 正常期：用原版动量计算（对数加权回归 + R²），32只ETF轮动
2. 震荡期：切换到防御ETF池（511880/511010/511220），用相同动量计算
3. 滤波器：正常期用拉普拉斯滤波，震荡期用高斯滤波（对价格序列预处理）
4. 盈利保护：5%回撤过滤
5. 退出震荡期：低点上涨3% + 回撤收窄 → 恢复权益轮动

关键改进：
- ✅ 滤波器真正生效（预处理价格序列）
- ✅ 震荡期不空仓，而是轮动防御ETF
- ✅ 正常期完全保持原版逻辑（不加滤波，收益不下降）
- ✅ 防御ETF也参与动量排名（不是死拿某一个）
"""
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def __init__(self):
        self.etf_datas = {}
        for etf in ALL_ETF_POOL + DEFENSIVE_ETF_POOL:
    ...
    def next(self):
        ...
        if self.current_filter == '震荡期':
            etf_pool = DEFENSIVE_ETF_POOL  # 只用防御ETF
        else:
            etf_pool = ALL_ETF_POOL  # 全部ETF
