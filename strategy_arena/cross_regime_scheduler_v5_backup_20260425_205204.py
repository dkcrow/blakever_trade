...
MAX_DRAWDOWN_HARD_LIMIT = 30  # 对齐规范：≥30%硬性淘汰
...
def _extract_strategy_from_code(ext: Dict, code: str, source: str) -> List[Dict]:
    ...
    has_momentum = bool(_re.search(r'momentum|rate.?of.?change|roc', code_lower))
...
def calculate_score(result: Dict, stress_result: Optional[Dict] = None,
    ...
    - 年化收益率（25%）：收益率/20% × 25
    - 夏普比率（25%）：夏普值/2.0 × 25
    - 最大回撤（20%）：≤10%:20分; 10-20%:15分; 20-25%:8分; ≥25%:0分淘汰
    - 盈亏比（15%）：盈亏比/2.0 × 15
    - 胜率（15%）：胜率/55% × 15
    ...
    # 硬性条件：最大回撤>30%淘汰（与strategy_ranker.py的score_max_drawdown一致）
    hard_fail = False
    fail_reason = ''
    if result.get('max_drawdown', 0) > MAX_DRAWDOWN_HARD_LIMIT:
        hard_fail = True
        fail_reason = f'最大回撤>{MAX_DRAWDOWN_HARD_LIMIT}%'
    elif result.get('annual_return', 0) < 0:
        hard_fail = True
        fail_reason = '年化收益为负'
...
def scan_strategies():
    ...
    # 5. 回测（v5升级：三市场分别回测）
    print(f"\n{'='*70}")
    print(f"  📋 步骤5: 执行回测 ({len(deduped_variants)}个策略 × 3个市场)")
    print(f"{'='*70}")

    passed_count = 0
    rejected_count = 0
    results = []
    new_best_by_market = {'US': None, 'HK': None, 'CN': None}  # 每个市场的新上榜策略

    if deduped_variants:
        for i, strategy in enumerate(deduped_variants):
            name = strategy['name']
            strategy_type = strategy.get('type', classify_strategy(name, '', strategy.get('desc', '')))
            print(f"\n  📋 [{i+1}/{len(deduped_variants)}] [{strategy_type}] {name}")
            ...
            if strategy_source == 'builtin':
                # 内置变体：发现时间 = 变体生成阶段总耗时均摊到每个变体
                discovery_time = search_stats.get('variant_generation_time', 0.5) / max(len(deduped_variants), 1)
            else:
                # 外部搜索策略：搜索阶段总耗时均摊到每个搜索结果
                search_total = search_stats.get('search_duration', search_stats.get('multi_source_stats', {}).get('duration', 5.0))
                discovery_time = search_total / max(search_stats.get('total_found', 1), 1)

            # 阶段2: 编码时间 = 策略从概念到可执行代码的耗时
            #   估算方式：按AI Agent代码生成速度(~50行/s)计算策略函数的编码耗时
            #   同族变体共享基础函数的编码时间
            code_start = time.time()
            
            # 获取策略函数的源代码行数来估算编码时间
            func_obj = strategy.get('func')
            code_lines = 0
            coding_overhead = 0.5  # 基础编码开销：参数验证、函数签名、导入等
            ...
            # AI Agent代码生成速度：约50行/s（含思考、编写、调试）
            code_gen_speed = 50  # 行/秒
            base_code_time = coding_overhead + code_lines / code_gen_speed
            
            # 统计同族变体数量（同一函数的不同参数组合）
            func_id = id(func_obj) if func_obj else 0
            if not hasattr(scan_strategies, '_func_family_count'):
                scan_strategies._func_family_count = {}
                scan_strategies._func_family_ids = {}
            if func_id not in scan_strategies._func_family_count:
                # 计算同族变体数（同一函数的不同参数组合）
                family_count = sum(1 for v in deduped_variants if id(v.get('func')) == func_id)
                scan_strategies._func_family_count[func_id] = max(family_count, 1)
            family_count = scan_strategies._func_family_count[func_id]
            
            # 同族变体共享编码时间：基础函数只写一次，参数变体各需少量修改
            if strategy_source == 'builtin':
                # 内置变体：基础函数编码时间均摊 + 参数调整时间(0.1s/变体)
                code_time = base_code_time / family_count + 0.1
            else:
                # 外部策略：适配转换 + 函数包装（通常需要更多调试）
                conversion_time_per = search_stats.get('conversion_time', 0.5) / max(search_stats.get('total_converted', 1), 1)
                code_time = base_code_time / family_count + conversion_time_per + 0.2  # +0.2s外部策略额外调试
            
            code_end = time.time()
            # 加上当前阶段的实际开销（指纹计算、去重验证等）
            code_time += (code_end - code_start)

            # 阶段3: 回测执行时间 = 信号生成 + 绩效计算（含主回测+压力回测+评分）
            ...
            try:
                ...
                print(f"    ❌ 信号生成异常: {e}")
                print(f"    ⏱️ 耗时: 发现{discovery_time:.2f}s + 编码{code_time:.2f}s + 回测{backtest_time:.2f}s = 总计{total_time:.2f}s")
                rejected_count += 1
                continue

            try:
                # ============================================================
                # v5升级：三市场分别回测（US / HK / CN）
                # ============================================================
                market_results = {}  # {market: {main_result, score_result, ...}}
                ...
                for market in ['US', 'HK', 'CN']:
                    ...
                    if market == 'US':
                        # === 美股：ETF轮动回测（原有逻辑） ===
                        main_result = run_backtest(close_prices, holding, MAIN_START, MAIN_END, risk_free_rate, 'US')
                        if main_result is None:
                            print(f"      ❌ 美股回测失败（数据不足）")
                            continue
                        stress_result = run_backtest(close_prices, holding, STRESS_START, STRESS_END, risk_free_rate, 'US')
                        score_result = calculate_score(main_result, stress_result, survivorship_bias)
                        market_results['US'] = {
                        ...
                        # 构建港股蓝筹close_prices（已映射为美股symbol名）
                        hk_close_dict = {}
                        for sym, df in hk_stock_data.items():
                            hk_close_dict[sym] = df['Close']
                        hk_close_prices = pd.DataFrame(hk_close_dict)
                        hk_close_prices = hk_close_prices.sort_index()
                        hk_close_prices = hk_close_prices.loc[HK_MAIN_START:HK_MAIN_END]
                        
                        if hk_close_prices.empty or len(hk_close_prices) < 100:
                            print(f"      ❌ 港股蓝筹数据不足")
                            continue
                        
                        # 港股策略信号生成：使用港股蓝筹的close_prices
                        try:
                            hk_holding = strategy['func'](hk_close_prices, **strategy['kwargs'])
                        except Exception as e:
                            print(f"      ❌ 港股信号生成异常: {e}")
                            continue
                        
                        # 港股回测（费率用港股费率）
                        hk_main_result = run_backtest(hk_close_prices, hk_holding, HK_MAIN_START, HK_MAIN_END, HK_RISK_FREE_RATE, 'HK')
                        if hk_main_result is not None:
                            hk_stress_result = run_backtest(hk_close_prices, hk_holding, HK_STRESS_START, HK_STRESS_END, HK_RISK_FREE_RATE, 'HK')
                            hk_score = calculate_score(hk_main_result, hk_stress_result, survivorship_bias=True)
                            market_results['HK'] = {
                                'main_result': hk_main_result,
                                'stress_result': hk_stress_result,
                                'score_result': hk_score,
                                'risk_free_rate': HK_RISK_FREE_RATE,
                                'survivorship_bias': True,  # 港股蓝筹池存在幸存者偏差
                            }
                        else:
                            print(f"      ❌ 港股回测失败")
                        ...
                        if not cn_etf_data:
                            print(f"      ⚠️ 无A股ETF数据，跳过")
                            continue
                        
                        # 构建A股ETF close_prices
                        cn_close_dict = {}
                        for sym, df in cn_etf_data.items():
                            cn_close_dict[sym] = df['Close']
                        cn_close_prices = pd.DataFrame(cn_close_dict)
                        cn_close_prices = cn_close_prices.sort_index()
                        cn_close_prices = cn_close_prices.loc[CN_MAIN_START:CN_MAIN_END]
                        
                        if cn_close_prices.empty or len(cn_close_prices) < 100:
                            print(f"      ❌ A股ETF数据不足")
                            continue
                        
                        # A股策略信号生成：使用A股ETF的close_prices
                        try:
                            cn_holding = strategy['func'](cn_close_prices, **strategy['kwargs'])
                        except Exception as e:
                            print(f"      ❌ A股信号生成异常: {e}")
                            continue
                        
                        # A股回测（费率用A股ETF费率，无印花税）
                        cn_main_result = run_backtest(cn_close_prices, cn_holding, CN_MAIN_START, CN_MAIN_END, CN_RISK_FREE_RATE, 'CN')
                        if cn_main_result is not None:
                            cn_stress_result = run_backtest(cn_close_prices, cn_holding, CN_STRESS_START, CN_STRESS_END, CN_RISK_FREE_RATE, 'CN')
                            cn_score = calculate_score(cn_main_result, cn_stress_result, survivorship_bias=True)
                            market_results['CN'] = {
                                'main_result': cn_main_result,
                                'stress_result': cn_stress_result,
                                'score_result': cn_score,
                                'risk_free_rate': CN_RISK_FREE_RATE,
                                'survivorship_bias': True,  # A股ETF池存在幸存者偏差
                            }
                        else:
                            print(f"      ❌ A股回测失败")
                
                # ============================================================
                # 汇总三个市场的回测结果，分别更新各市场排行榜
                # ============================================================
                best_market = None
                best_score = 0
                ...
                if 'US' in market_results:
                    try:
                        batch_result = run_batch_backtest(
                            all_market_data, strategy['func'], strategy['kwargs'], risk_free_rate)
                        ...
                        print(f"    ⚠️ 多标的回测异常（不影响ETF评分）: {e}")

                # 汇总结果（以美股为主，附带三市场得分对比）
