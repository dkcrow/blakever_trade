...
def load_etf_data(code):
    ...
    try:
        ...
        # ✅ 关键修复：只传2026年数据给Backtrader
        start = pd.Timestamp('2026-01-01')
        end = pd.Timestamp('2026-05-20')
        df_2026 = df[(df.index >= start) & (df.index <= end)].copy()
        
        if len(df_2026) < 10:
            print(f"  ⚠️ 2026年数据不足: {len(df_2026)}行")
            return None
        
        print(f"      2026年: {len(df_2026)}行 ({df_2026.index[0].date()} ~ {df_2026.index[-1].date()})")
        ...
        if 'volume' not in df_2026.columns:
            df_2026['volume'] = 0
        
        return df_2026[['open', 'high', 'low', 'close', 'volume']].dropna()
