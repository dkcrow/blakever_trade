...
class QixingSanmaV5(bt.Strategy):
    params = (
...
def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    print(f'\n{"="*70}')
    print(f'三马七星策略 - 美股个股版本 V5（Top2持仓）')
    print(f'{"="*70}')
    print(f'参数: 短期{SHORT_LOOKBACK}日/长期{LONG_LOOKBACK}日/止损{STOP_LOSS_PCT*100:.0f}%/得分>{MIN_SCORE}/持仓{MAX_POSITIONS}只')
    ...
    cerebro.addstrategy(QixingSanmaV5)
