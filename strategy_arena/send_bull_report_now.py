...
# 排行榜前5
top5 = leaderboard[:5] if leaderboard else []
...
def build_top5_detail():
    if not top5:
        return '<p style="color:#888;font-size:13px;">暂无上榜策略</p>'
    ...
    for i, s in enumerate(top5):
        ...
        wr = s.get('win_rate', 0)
        trades = s.get('annual_trades', 0)
        params = s.get('params', '')
        stress = s.get('stress_test', '')
        desc = s.get('description', '')
        robust = s.get('robust', '')
        robust_icon = '✅' if robust else '❌'
        ...
        <div style="border:2px solid {rank_colors[i]};border-radius:10px;margin-bottom:15px;overflow:hidden;">
            <div style="background:{rank_colors[i]}20;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;">
                <span style="font-size:18px;">{rank_icons[i]} 第{rank}名: {name} {market_flag} ({mkt})</span>
                <span style="font-size:22px;font-weight:bold;color:{rank_colors[i]};">{score:.2f}分</span>
            </div>
            <div style="padding:15px 20px;">
                <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px;">
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        ...
                        <div style="font-size:18px;font-weight:bold;color:{return_color(ret)};">{ret:.2f}%</div>
                    </div>
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        <div style="font-size:11px;color:#888;">夏普比率</div>
                        <div style="font-size:18px;font-weight:bold;">{sharpe:.2f}</div>
                    </div>
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        <div style="font-size:11px;color:#888;">最大回撤</div>
                        <div style="font-size:18px;font-weight:bold;color:{dd_color(dd)};">{dd:.2f}%</div>
                    </div>
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        <div style="font-size:11px;color:#888;">盈亏比</div>
                        <div style="font-size:18px;font-weight:bold;">{pf:.2f}</div>
                    </div>
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        <div style="font-size:11px;color:#888;">胜率</div>
                        <div style="font-size:18px;font-weight:bold;">{wr:.1f}%</div>
                    </div>
                    <div style="flex:1;min-width:120px;background:#f8f9fa;border-radius:6px;padding:8px 12px;">
                        <div style="font-size:11px;color:#888;">年交易次数</div>
                        <div style="font-size:18px;font-weight:bold;">{trades:.1f}</div>
                    </div>
                </div>
                <div style="display:flex;gap:15px;font-size:13px;color:#555;">
                    <div><b>策略类型:</b> {stype}</div>
                    <div><b>策略参数:</b> {params}</div>
                </div>
                <div style="display:flex;gap:15px;font-size:13px;color:#555;margin-top:6px;">
                    <div><b>压力测试:</b> {stress}</div>
                    <div><b>跨周期鲁棒:</b> {robust_icon}</div>
                </div>
                <div style="font-size:13px;color:#777;margin-top:6px;"><b>描述:</b> {desc}</div>
            </div>
        </div>'''
...
<div style="background:linear-gradient(135deg,#2c3e50,#3498db);color:white;padding:30px;">
    <h1 style="margin:0;font-size:24px;">🤖 牛市/全局策略回测定时报告</h1>
...
<div style="display:flex;flex-wrap:wrap;padding:20px;gap:12px;">
    ...
    <div style="flex:1;min-width:140px;background:#ffebee;border-radius:8px;padding:15px;text-align:center;">
        <div style="font-size:28px;font-weight:bold;color:#e74c3c;">0</div>
...
<div style="padding:0 20px 10px;">
    <div style="background:#e3f2fd;border-left:4px solid #1976d2;padding:12px 15px;border-radius:0 8px 8px 0;">
        <b>🇺🇸 美股扫描:</b> 发现13 → 去重后9 → 回测9 → 有评分0 / 零分9(回撤>25%)
...
<div style="background:#f8f9fa;padding:15px 20px;text-align:center;color:#888;font-size:12px;border-top:1px solid #eee;">
    Blakever 策略回测系统 · 牛市/全局回测定时报告 · {now.strftime('%Y-%m-%d %H:%M')}
</div>
...
msg["Subject"] = f"【牛市/全局回测定时报告】{now.strftime('%Y-%m-%d')} {now.strftime('%H:%M')}"
msg["From"] = SENDER
msg["To"] = RECEIVER
...
print(f"   标题: 【牛市/全局回测定时报告】{now.strftime('%Y-%m-%d')} {now.strftime('%H:%M')}")
