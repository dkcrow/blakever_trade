...
拉普拉斯策略 - 改进版（采纳改进点1+3）
改进点：
1. 震荡期强制避险：非防御ETF直接过滤，只有防御ETF参与排名
3. 动态盈利保护：根据20日波动率调整回撤阈值（3%-8%）

目标：保持滤波器切换生效 + 震荡期真正避险 + 不降低收益
"""
...
DEFENSIVE_ETF_LIST = ['511880', '511010', '511220']
...
PROFIT_PROTECTION_THRESHOLD = 0.05  # 基准阈值，会动态调整
...
MA_PERIOD = 20
BIAS_ENTER_THRESHOLD = 0.10
BIAS_EXIT_THRESHOLD = 0.05
RSI_OVERBOUGHT = 75
RSI_PULLBACK = 60
MAX_RANGE_BOUND_DAYS = 30
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def calculate_momentum(self, etf):
        ...
        try:
            ...
            if self.current_filter == '震荡期':
                # 非防御ETF直接过滤，不参与排名
                if etf not in DEFENSIVE_ETF_LIST:
                    return None
                # 防御ETF用高斯滤波
            ...
            if self.p.enable_profit_protection:
                ...
                # 动态阈值：根据20日波动率调整
                if len(closes) >= 20:
                    volatility = np.std(np.diff(np.log(closes[-20:])))
                    dynamic_threshold = max(0.03, min(0.08, volatility * 10))  # 3%-8%之间
                else:
                    dynamic_threshold = PROFIT_PROTECTION_THRESHOLD
    ...
    def check_range_bound_mode(self):
        ...
        # 退出震荡期（简化：乖离率回归）
        should_exit = False
        if abs(bias) < BIAS_EXIT_THRESHOLD:
            should_exit = True
        ...
        if should_enter and self.current_filter == '正常期':
            ...
            self.range_bound_days = 0
            print(f"[{self.last_date}] 进入震荡期 → 仅防御ETF参与排名")
        elif should_exit and self.current_filter == '震荡期':
            self.current_filter = '正常期'
            print(f"[{self.last_date}] 退出震荡期 → 恢复全部ETF排名")
