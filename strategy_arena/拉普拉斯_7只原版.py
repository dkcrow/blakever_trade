...
# -*- coding: utf-8 -*-
"""
拉普拉斯高斯策略 - 对齐聚宽原版V1.7.2（7只原始ETF池）
关键：用 g.etf_pool_bak 而不是 g.etf_pool
参考数据：2026.2.1~4.16 收益115% 胜率68% 盈亏比9.3
"""
...
DEFENSIVE_ETF = '511220'         # 防御ETF（城投债）
...
# 成交量过滤
ENABLE_VOLUME_CHECK = False       # Backtrader版暂不支持实时成交量
VOLUME_LOOKBACK = 5
VOLUME_THRESHOLD = 2
...
RISK_BENCHMARK = '159915'         # 风险基准ETF（创业板）
...
class LaplaceGaussianStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        print(f"策略初始化完成，加载 {len(self.etf_datas)} 只ETF（原版7只池）")
    ...
    def _check_defensive(self):
        """无合格ETF时，检查是否需要持有防御ETF"""
        current_pos = None
        ...
        if current_pos != self.p.defensive_etf:
            if current_pos:
                ...
                print(f"[{self.data.datetime.date(0)}] 卖出: {current_pos}（无合格ETF）")
            self._buy_defensive()
    ...
    def calculate_all_rankings(self):
        ...
        for etf in ETF_POOL_BAK:
            ...
            if m and MIN_SCORE_THRESHOLD <= m['score'] <= MAX_SCORE_THRESHOLD:
                metrics.append(m)
...
def run_backtest():
    ...
    print(f"成功加载 {loaded} 只ETF数据（原版7只池）")
    ...
    print("="*80)
    print("开始回测: 拉普拉斯高斯策略（聚宽原版V1.7.2 - 7只ETF池）")
    print("="*80)
    print(f"回测时间: 2026-02-01 ~ 2026-04-16（75天）")
    print(f"ETF池: {loaded}只（原版g.etf_pool_bak）")
