...
# -*- coding: utf-8 -*-
"""
三马七星策略 - 美股个股版本 V8（ATR 3倍 + 时间止损）
=====================================================
核心改进：
1. ATR 3倍动态止损（更宽，让利润奔跑）
2. 时间止损：持仓超过60天强制退出
3. 其他参数同V7
"""
...
SHORT_LOOKBACK = 20
LONG_LOOKBACK = 60
ATR_MULTIPLIER = 3.0   # ATR 3倍（更宽）
ATR_PERIOD = 14
TIME_STOP_DAYS = 60    # 时间止损：60天
MIN_SCORE = 0.15
MAX_POSITIONS = 2
...
class QixingSanmaV8(bt.Strategy):
    params = (
    ...
    def next(self):
        ...
        for name, pos in list(self.holdings.items()):
            ...
            time_stop = bars_held >= self.p.time_stop_days
            ...
            if price <= stop_price:
                ...
                print(f'  [ATR_STOP] {name}: {price:.2f} (pnl:{pnl:.1f}%) atr={atr:.2f}')
            ...
            if time_stop:
                self.close(data=d)
                pnl = (price / entry_price - 1) * 100
                self.trade_log.append({'date': self.datas[0].datetime.date(0), 'action': 'TIME_STOP', 'stock': name, 'price': price, 'pnl_pct': pnl})
                print(f'  [TIME_STOP] {name}: {price:.2f} (pnl:{pnl:.1f}%) held {bars_held} days')
                to_remove.append(name)
                continue
        ...
        for name in to_buy:
            ...
            if size > 0:
                ...
                self.holdings[name] = {'entry_price': d.close[0], 'entry_bar': len(d)}
...
def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    print(f'\n{"="*70}')
    print(f'三马七星策略 - 美股个股版本 V8（ATR 3倍 + 时间止损）')
    print(f'{"="*70}')
    print(f'参数: 短期{SHORT_LOOKBACK}日/长期{LONG_LOOKBACK}日/ATR{ATR_MULTIPLIER}倍/时间止损{TIME_STOP_DAYS}天/得分>{MIN_SCORE}/持仓{MAX_POSITIONS}只')
    ...
    cerebro.addstrategy(QixingSanmaV8)
