...
def show(strat, label, cash=100000.0):
    ...
    try:
        dd_val = dd.get('max', {}).get('drawdown', 0.0)
...
class Sanma(bt.Strategy):
    params = (('short_lb', 25), ('long_lb', 250), ('top_n', 1))
    def __init__(s):
        s.etfs = {}
    ...
    def next(s):
        ...
        for etf in SANMA_ETFs:
            ...
            if len(closes) >= 5:
                ...
                for i in range(len(closes)-5, len(closes)-1):
                    if closes[i] > 0 and closes[i+1] / closes[i] < 0.95:
                        bad = True; break
                if bad: continue
        ...
        if not scores:
            ...
            target_etfs = set(e['etf'] for e in scores[:s.p.top_n])  # Hold Top 3
        ...
        for etf in list(s.current_etfs):
            if etf not in target_etfs:
                ...
                if d and s.getposition(d).size > 0:
                    ...
                    print('  Close %s (size=%d)' % (etf, s.getposition(d).size))
                s.current_etfs.discard(etf)
        # Buy new ETFs in target
...
if __name__ == '__main__':
    ...
    print('=' * 60)
    run(Sanma, 'Strategy A: Sanma (25 ETFs, pure momentum Top1)', SANMA_ETFs, fd, td)
    run(Laplace, 'Strategy B: Laplace (32 ETFs, Juquan original)', LAPLACE_ETFs, fd, td)
    print('\n' + '=' * 60)
    print('  Summary')
    print('=' * 60)
    print('  Sanma:   25 ETFs, pure momentum, Top1, defense=511880')
