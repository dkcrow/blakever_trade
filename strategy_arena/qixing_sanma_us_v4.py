...
class QixingSanmaV4(bt.Strategy):
    params = (
    ...
    def next(self):
        ...
        if size > 0:
            ...
            self.holdings[top_stock] = {'entry_price': d.close[0]}
            self.trade_log.append({'date': self.datas[0].datetime.date(0), 'action': 'BUY', 'stock': top_stock, 'price': d.close[0], 'size': size})
            print(f'  [BUY] {top_stock}: {d.close[0]:.2f} x{size} (score:{top_score:.4f})')

def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    print(f'\n{"="*70}')
    print(f'三马七星策略 - 美股个股版本 V4（集中持仓+趋势跟踪）')
    print(f'{"="*70}')
    print(f'参数: 短期{SHORT_LOOKBACK}日/长期{LONG_LOOKBACK}日/止损{STOP_LOSS_PCT*100:.0f}%/最小得分{MIN_SCORE}')
    ...
    cerebro.addstrategy(QixingSanmaV4)
