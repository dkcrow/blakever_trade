"""
拉普拉斯盘中监控 v13 - 修复综合得分重复bug
关键修复：1) 移除所有emoji避免GBK编码错误 2) 修复get_rankings()中long=short问题 3) 修复y变量名拼写错误
"""
import pandas as pd
import numpy as np
...
def generate_html(results, rank_changes):
  ...
  '''
    ...
    top3 = results[:3]
    top_summary = ' | '.join([
        f'1 {r["name"]}' if i==0 else (f'2 {r["name"]}' if i==1 else f'3 {r["name"]}')
