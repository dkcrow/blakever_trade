...
def get_westock_prices(etf_codes, n=1, raise_on_fail=False):
    """
    通过westock-data skill获取ETF实时最新价。
    
    Args:
        etf_codes: ETF代码列表
        n: 获取K线天数
        raise_on_fail: 如果为True，任何ETF获取失败则抛出异常
    
    Returns:
        dict: {etf_code: price}，如果raise_on_fail=False则返回部分成功结果
    """
    if not etf_codes:
        return {}
    price_map = {}
    ...
    for code in etf_codes:
        if code.endswith('_XSHE'):
            code_map[code] = 'sz' + code.replace('_XSHE', '')
        elif code.endswith('_XSHG'):
            code_map[code] = 'sh' + code.replace('_XSHG', '')
        else:
            code_map[code] = code.lower()
    
    westock_codes = ','.join(code_map.values())
    
    try:
        import subprocess
        cmd = f'npx -y westock-data-skillhub@1.0.3 kline {westock_codes} --period day --limit {n}'
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30, shell=True, encoding='utf-8', errors='replace')
        if result.returncode == 0:
            lines_out = result.stdout.strip().split('\n')
            # 批量查询格式: [Batch] 状态: success | 总数: X | 成功: X | 失败: 0
            # 表头: | symbol | date | open | last | high | low | volume | amount | exchange |
            # 数据行: | sz159915 | 2026-05-13 | 3.91 | 4.00 | 4.01 | 3.90 | ... |
            data_start = 0
            ...
            # 建立 wscode -> etf_code 的反向映射
            wscode_to_etf = {v: k for k, v in code_map.items()}
            
            for line in lines_out[data_start:]:
                if line.strip().startswith('|') and not line.strip().startswith('| symbol'):
                    parts = [p.strip() for p in line.split('|')]
                    if len(parts) >= 5:
                        try:
                            wscode = parts[1]
                            last_price = float(parts[4])  # last最新价
                            etf_code = wscode_to_etf.get(wscode)
                            if etf_code:
                                price_map[etf_code] = last_price
                        except (ValueError, IndexError):
                            pass
    except Exception as e:
        if raise_on_fail:
            raise RuntimeError(f"westock-data调用失败: {e}")
    
    # 检查是否所有ETF都获取成功
    if raise_on_fail:
        failed_codes = [c for c in etf_codes if c not in price_map]
        if failed_codes:
            raise RuntimeError(f"以下ETF实时价格获取失败: {failed_codes}")
...
def get_etf_score_ranking(close_prices, etf_pool, safe_assets,
    ...
    Args:
        require_live_prices: 如果为True，强制获取实时价格计算评分，失败则抛出异常
    """
    ...
    for asset in pool_in_data:
        ...
        results.append({
            ...
            'price': None,  # 稍后统一填充实时价格
            'dropped': dropped,
        })

    # === 统一获取实时价格（强制要求）===
    if require_live_prices:
        try:
            live_prices = get_westock_prices(pool_in_data, raise_on_fail=True)
            for r in results:
                r['price'] = live_prices.get(r['code'], 0.0)
        except Exception as e:
            raise RuntimeError(f"实时价格获取失败，无法计算评分: {e}")
    else:
        # 降级模式：尝试获取，失败用CSV最后价
        live_prices = get_westock_prices(pool_in_data, raise_on_fail=False)
        for r in results:
            r['price'] = live_prices.get(r['code'], close_prices[r['code']].iloc[loc])
    ...
    lines.append("  %s" % ("-" * 60))
    lines.append("  %-8s %-6s %8s %8s %8s %8s   %s" % ("代码", "名称", "最新价", "综合分", "短期分", "长期分", "状态"))
    lines.append("  %s" % ("-" * 60))
    for r in results:
        lines.append("  %-8s %-6s %8.3f %8.4f %8.4f %8.4f   %s"
                     % (r['code'], r['name'], r['price'] or 0.0, r['combined'], r['short'], r['long'], r['flag']))
    lines.append("  %s" % ("-" * 60))
    winner = next((r for r in results if r['combined'] == max_combined and not r['dropped']), None)
    if winner:
        lines.append("  \U0001f3af 建议: %s（最新价 %.3f，综合分 %.4f）" % (winner['name'], winner['price'], winner['combined']))
...
# 每日模拟盘运行
# ================================================================
def run_daily_simulation(target_date=None, no_trade=False):
    """
    运行每日A股模拟盘

    Args:
        target_date: 目标日期，格式'YYYY-MM-DD'，默认为今天
        no_trade: 如果为True，只生成报告不执行换仓交易
    """
    ...
    # === 数据新鲜度检查 ===
    latest_csv_date = close_prices.index[-1]
    days_stale = (today - latest_csv_date).days
    if days_stale > 1:
        print(f"\n⚠️ 本地CSV数据滞后{days_stale}天（最后更新：{latest_csv_date.strftime('%Y-%m-%d')}），尝试用westock实时价格补全...")
        # 用westock实时价格补全缺失的最新数据
        try:
            all_pool_codes = list(CN_ETF_NAMES.keys())
            live_prices = get_westock_prices(all_pool_codes, raise_on_fail=True)
            # 为每只ETF添加今天的数据行
            for code in all_pool_codes:
                if code in close_prices.columns and code in live_prices:
                    price = live_prices[code]
                    # 用最新价作为open/high/low/close（简化处理）
                    close_prices.loc[today, code] = price
            # 重新排序索引
            close_prices = close_prices.sort_index()
            print(f"✅ 已用westock实时价格补全{len(live_prices)}只ETF的{today.strftime('%Y-%m-%d')}数据")
        except Exception as e:
            error_msg = f"本地CSV数据滞后{days_stale}天，且westock补全失败: {e}"
            print(f"\n❌ {error_msg}")
            send_error_email(error_msg, today)
            return None

    # 3.5 打印ETF评分排名（强制获取实时价格）
    try:
    ...
    # 如果设置了no_trade，强制不执行换仓
    if no_trade:
        need_rebalance = False
        operation = '持有（仅观察，未执行换仓）'
        print(f"\n⚠️ 仅观察模式：不执行任何交易操作")
    else:
        operation = '换仓' if need_rebalance else '持有'

    # === 强制获取所有ETF实时价格（用于交易和净值计算）===
    all_pool_codes = list(CN_ETF_NAMES.keys())
    try:
        live_prices = get_westock_prices(all_pool_codes, raise_on_fail=True)
        print(f"\n✅ 实时价格获取成功: {len(live_prices)}/{len(all_pool_codes)}只ETF")
    except Exception as e:
        error_msg = f"实时价格获取失败，无法执行交易和净值计算: {e}"
        print(f"\n❌ {error_msg}")
        send_error_email(error_msg, today)
        return None

    live_current_price = live_prices.get(current_holding, 0.0)
    current_price = live_current_price  # 强制使用实时价，不用CSV旧数据
    ...
    if need_rebalance:
        ...
        # 估算手续费
        if old_etf and old_etf in live_prices and live_prices[old_etf] > 0:
            old_price = live_prices[old_etf]
        elif old_etf and old_etf in latest_row:
        ...
        # 持有不变（或仅观察模式）
        if no_trade and current_holding != prev_holding:
            operation_detail = f'【观察】策略建议换仓至{current_holding_name}，但未执行交易'
        else:
            operation_detail = f'继续持有{current_holding_name}'
        print(f"  操作明细: {operation_detail}")

        # 更新市值（强制用实时价，禁止用CSV旧数据）
        account.hold_position(today.strftime('%Y-%m-%d'), live_prices)

    # 7. 构建每日操作明细（强制用实时价）
    position_details = account.get_position_details(live_prices)
    # 转换为以etf_code为key的字典
...
'''
    ...
    if _ranking_raw:
        # 解析文本排名，生成HTML表格
        _hrows = []
        _hrows.append('<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">')
        _hrows.append('<table style="width:100%;min-width:380px;border-collapse:collapse;font-size:12px;">')
        _hrows.append('<thead><tr style="background:#1f2937;">')
        _hrows.append('  <th style="padding:6px 6px;text-align:left;color:#9ca3af;">代码</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:left;color:#9ca3af;">名称</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:right;color:#9ca3af;">最新价</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:right;color:#f97316;">综合分</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:right;color:#9ca3af;">短期分</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:right;color:#9ca3af;">长期分</th>')
        _hrows.append('  <th style="padding:6px 6px;text-align:center;color:#9ca3af;">状态</th>')
        _hrows.append('</tr></thead><tbody>')
        for _line in _ranking_raw.strip().split('\n'):
            _line = _line.strip()
            if not _line or '---' in _line or '📊' in _line or '🎯' in _line:
                continue
            # 识别数据行: 跳过表头(代码开头)和非数据行
            _parts = _line.split()
            if len(_parts) >= 7:
                _code = _parts[0]
                _name = _parts[1]
                ...
                _short = _parts[4]
                _long = _parts[5]
                _status = ' '.join(_parts[6:])
                _is_winner = '建议持有' in _status
                _is_dropped = '跌幅' in _status
                _bg = '#1a1a2e' if _is_winner else ('#1f2937' if _is_dropped else 'transparent')
                try:
                    _c = '#22c55e' if float(_combined) > 0 else '#ef4444'
                except ValueError:
                    continue
                _hrows.append('<tr style="border-bottom:1px solid #2d2d44;background:%s;">' % _bg)
                _hrows.append('  <td style="padding:5px 6px;color:#e5e7eb;font-size:11px;">%s</td>' % _code)
                _hrows.append('  <td style="padding:5px 6px;color:#e5e7eb;font-weight:600;font-size:11px;">%s</td>' % _name)
                _hrows.append('  <td style="padding:5px 6px;text-align:right;color:#e5e7eb;font-size:11px;">%s</td>' % _price)
                _hrows.append('  <td style="padding:5px 6px;text-align:right;color:%s;font-weight:700;font-size:11px;">%s</td>' % (_c, _combined))
                _hrows.append('  <td style="padding:5px 6px;text-align:right;color:#9ca3af;font-size:11px;">%s</td>' % _short)
                _hrows.append('  <td style="padding:5px 6px;text-align:right;color:#9ca3af;font-size:11px;">%s</td>' % _long)
                _hrows.append('  <td style="padding:5px 6px;text-align:center;font-size:11px;">%s</td>' % _status)
                _hrows.append('</tr>')
        _hrows.append('</tbody></table></div>')
        score_ranking_html = '\n'.join(_hrows)
    else:
...
<div style="max-width:720px;margin:0 auto;padding:16px;">
  ...
  <div style="text-align:center;color:#6b7280;font-size:11px;margin-top:16px;padding:16px 0;">
    <p>七星高照A股模拟盘 | 本金¥{INIT_CAPITAL:,.0f} | 数据来源：westock-data实时价格 + 本地CSV历史数据</p>
...
def send_error_email(error_msg, today):
    """数据获取失败时发送错误提示邮件"""
    html_content = f'''<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>七星高照模拟盘 - 数据获取失败</title></head>
<body style="background:#0c0c14;color:#e5e7eb;font-family:'PingFang SC','Microsoft YaHei',sans-serif;margin:0;padding:16px;">
<div style="max-width:600px;margin:0 auto;">
  <div style="text-align:center;padding:24px 0;">
    <h1 style="color:#ef4444;font-size:20px;">⚠️ 数据获取失败</h1>
    <p style="color:#9ca3af;font-size:13px;">{today.strftime('%Y-%m-%d')}</p>
  </div>
  <div style="background:#1a1a2e;border:1px solid #ef4444;border-radius:12px;padding:18px;margin:16px 0;">
    <p style="color:#ef4444;font-size:14px;font-weight:600;">今日模拟盘任务中止，原因如下：</p>
    <p style="color:#e5e7eb;font-size:13px;margin-top:12px;">{error_msg}</p>
    <p style="color:#9ca3af;font-size:12px;margin-top:16px;">
      可能原因：<br>
      • westock-data 服务暂时不可用<br>
      • 网络连接问题<br>
      • 本地CSV数据未及时更新<br><br>
      建议：请检查网络连接和数据源状态，稍后手动重试。
    </p>
  </div>
  <div style="text-align:center;color:#6b7280;font-size:11px;margin-top:16px;">
    <p>七星高照A股模拟盘 | 生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
  </div>
</div>
</body>
</html>'''
    ...
    msg = MIMEMultipart('alternative')
    msg['Subject'] = f'【七星高照A股模拟盘】{today.strftime("%Y-%m-%d")} ⚠️ 数据获取失败'
    msg['From'] = '848786642@qq.com'
    msg['To'] = '848786642@qq.com'
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='七星高照A股每日模拟盘')
    parser.add_argument('--no-trade', action='store_true', help='仅生成报告，不执行换仓交易')
    parser.add_argument('--date', type=str, default=None, help='指定日期 (YYYY-MM-DD)')
    args = parser.parse_args()
    
    run_daily_simulation(target_date=args.date, no_trade=args.no_trade)
