...
# -*- coding: utf-8 -*-
"""
三马七星策略 - 美股个股版本 V6（移动止损版）
===================================================
核心改进：
1. 移动止损：最高价回撤10%触发（让利润奔跑）
2. 硬止损保持20%（防止大亏）
3. 其他参数同V5
"""
...
LONG_LOOKBACK = 60
STOP_LOSS_PCT = 0.20
TRAILING_STOP_PCT = 0.10  # 移动止损：最高价回撤10%
MIN_SCORE = 0.15
MAX_POSITIONS = 2
...
class QixingSanmaV6(bt.Strategy):
    params = (
    ...
    def next(self):
        ...
        for name, pos in list(self.holdings.items()):
            ...
            if price > highest_price:
                highest_price = price
                self.holdings[name]['highest_price'] = highest_price
            ...
            if price / entry_price - 1 <= -self.p.stop_loss_pct:
                ...
                print(f'  [STOP] {name}: {price:.2f} (pnl:{pnl:.1f}%) hard stop')
                to_remove.append(name)
                continue
            
            # 移动止损：最高价回撤10%
            if price / highest_price - 1 <= -self.p.trailing_stop_pct:
                self.close(data=d)
                pnl = (price / entry_price - 1) * 100
                self.trade_log.append({'date': self.datas[0].datetime.date(0), 'action': 'TRAIL', 'stock': name, 'price': price, 'pnl_pct': pnl})
                print(f'  [TRAIL] {name}: {price:.2f} (pnl:{pnl:.1f}%) trailing stop from {highest_price:.2f}')
                to_remove.append(name)
                continue
        ...
        for name in to_buy:
            ...
            if size > 0:
                ...
                self.holdings[name] = {'entry_price': d.close[0], 'highest_price': d.close[0]}
...
def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    print(f'\n{"="*70}')
    print(f'三马七星策略 - 美股个股版本 V6（移动止损版）')
    print(f'{"="*70}')
    print(f'参数: 短期{SHORT_LOOKBACK}日/长期{LONG_LOOKBACK}日/硬止损{STOP_LOSS_PCT*100:.0f}%/移动止损{TRAILING_STOP_PCT*100:.0f}%/得分>{MIN_SCORE}/持仓{MAX_POSITIONS}只')
    ...
    cerebro.addstrategy(QixingSanmaV6)
