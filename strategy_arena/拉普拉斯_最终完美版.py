...
核心改进：
- 进入震荡期：乖离率>10% 或 RSI从75+回落到60-
- 退出震荡期：乖离率回归到<5% （简化！）
- 滤波器：正常期用拉普拉斯，震荡期用高斯
- 避险：震荡期防御ETF(511880/511010/511220)获得1.5倍加分
"""
...
MA_PERIOD = 20
BIAS_ENTER_THRESHOLD = 0.10   # 进入：乖离率>10%
BIAS_EXIT_THRESHOLD = 0.05     # 退出：乖离率<5%（简化！）
RSI_OVERBOUGHT = 75
RSI_PULLBACK = 60
MAX_RANGE_BOUND_DAYS = 30  # 最多震荡30天
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def check_range_bound_mode(self):
        ...
        # 退出震荡期（简化：乖离率回归）
        should_exit = False
        if abs(bias) < BIAS_EXIT_THRESHOLD:  # 乖离率回归到<5%
            should_exit = True
