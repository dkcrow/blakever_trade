...
七星拉普拉斯高斯策略 - 精简优化版
核心改进：
1. 正常期/震荡期滤波器真正生效（拉普拉斯 vs 高斯）
2. 震荡期转入防御ETF（511880）避险
3. 退出震荡期条件补全（低点上涨+回撤收窄）
4. 最小持仓周期（避免频繁交易）
"""
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def __init__(self):
        self.etf_datas = {}
        for etf in self.params.etf_pool if hasattr(self.params, 'etf_pool') else ETF_POOL:
    ...
    def next(self):
        ...
        # 检查最小持仓周期
        if current_bar - self.last_switch_bar < MIN_HOLD_BARS:
            return
    ...
    def _rebalance(self, targets):
        ...
        for d in self.datas:
            ...
            if self.getposition(d).size > 0 and etf not in targets:
                datas_to_close.append(d)
        
        for d in datas_to_close:
            self.close(d)
            print(f"[{self.last_date}] 卖出: {d._name}")
    ...
    def calculate_momentum(self, etf):
        ...
        try:
            ...
            # 根据市场状态选择滤波器
            if self.current_filter == '震荡期':
                filtered = gaussian_filter(np.array(closes), sigma=GAUSSIAN_SIGMA)
                recent = filtered[-(self.p.lookback_days + 1):]
            else:
                filtered = laplace_filter(np.array(closes), s=LAPLACE_S_PARAM)
                recent = filtered[-(self.p.lookback_days + 1):]
    ...
    def check_range_bound_mode(self):
        ...
        if should_enter and self.current_filter == '正常期':
            ...
            self.range_bound_days = 0
            print(f"[{self.last_date}] 进入震荡期（高斯滤波，转防御ETF）")
