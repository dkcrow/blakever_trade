...
# -*- coding: utf-8 -*-
"""
三马七星策略 - 美股个股版本 V7（ATR动态止损版）
===================================================
核心改进：
1. ATR动态止损：2倍ATR（根据波动率自适应）
2. 取消固定百分比止损
3. 其他参数同V5
"""
...
class QixingSanmaV11(bt.Strategy):
    params = (
    ...
    def next(self):
        ...
        for name, pos in list(self.holdings.items()):
            ...
            if price <= stop_price:
                ...
                print(f'  [ATR_STOP] {name}: {price:.2f} (pnl:{pnl:.1f}%) atr={atr:.2f} stop={stop_price:.2f}')
...
def run_backtest(start_date='2016-05-19', end_date='2026-05-18'):
    print(f'\n{"="*70}')
    print(f'三马七星策略 - 美股个股版本 V11（V7 + Top5持仓）')
    print(f'{"="*70}')
    ...
    cerebro.addstrategy(QixingSanmaV11)
