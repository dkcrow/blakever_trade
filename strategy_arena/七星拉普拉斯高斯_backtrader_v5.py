...
# 美股ETF列表（从文件列表推断）
US_ETF_LIST = ['AGG', 'GLD', 'IEF', 'QQQ', 'SH', 'SHY', 'SPY', 'SPY_RV', 'TLT', 'VEA']

# 防御ETF（债券/货币）
DEFENSIVE_ETFs = ['SHY', 'IEF', 'TLT', 'AGG']  # 短期/长期国债，防御型
...
class QixingLaplaceGaussian(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 防御ETF列表
        self.defensive_list = [
            name for name in self.stock_datas.keys()
            if any(def_name in name for def_name in DEFENSIVE_ETFs)
        ]
    ...
    def next(self):
        ...
        if self.current_filter == '震荡期' and self.p.enable_defensive_switch and self.defensive_list:
            # 震荡期：切换到防御ETF
            targets = [self.defensive_list[0]]  # 持有一只防御ETF
        else:
            # 正常期：持有Top N
        ...
        for name in targets:
            ...
            try:
                ...
                if math.isnan(current_price) or current_price <= 0:
                    print(f"[{dt}] {name} 价格无效: {current_price}")
                    continue
                
                pos = self.getposition(d).size
                cur_val = pos * current_price
                
                if abs(cur_val - target_val) > target_val * 0.05 or pos == 0:
                    if math.isnan(target_val) or target_val <= 0:
                        print(f"[{dt}] {name} target_val无效: {target_val}")
                        continue
                    size = int(target_val / current_price / 100) * 100
                    if size > 0 and size != pos:
                        self.order_target_size(d, size)
                        print(f"[{dt}] 买入 {name} @ {current_price:.3f}, size={size}")
            except Exception as e:
                print(f"[{dt}] {name} 错误: {e}")
                continue
...
def run_laplace_backtest():
    ...
    print("\n" + "="*70)
    print("七星拉普拉斯高斯策略 - Backtrader回测（美股ETF版）")
    print("="*70)
