#!/usr/bin/env python3
"""
综合数据补充脚本 v2：修复港股yfinance代码转换，增加商品数据

修复：
- 港股yfinance代码转换: hk00001 -> 0001.HK (去除hk前缀，补零到4位)
- 商品数据直接执行不依赖后台

数据源限制：
- JQData试用账号: 仅限2025-01-14~2026-01-21, 无法获取10年数据
- yfinance: 可获取10年数据，但可能被限流
- westock-data: 港股/美股最多2000条
"""
...
def hk_code_to_yfinance(hk_code):
    """港股代码转换: hk00001 -> 0001.HK"""
    # 去掉hk前缀
    num_part = hk_code.lower().replace('hk', '')
    # 补零到4位
    yf_code = num_part.zfill(4) + '.HK'
    return yf_code


def fetch_hk_with_yfinance(hk_code, start_date, end_date):
    """用yfinance获取港股数据"""
    yf_symbol = hk_code_to_yfinance(hk_code)
    print(f"    yfinance代码: {yf_symbol}")
    return fetch_yfinance_kline(yf_symbol, start_date, end_date)


# ============================================================
# 任务1: 中证500成分股 (JQData - 仅2025-01-14~2026-01-21)
# ============================================================
...
# ============================================================
# 任务2: 港美股补充至近10年 (yfinance)
# ============================================================

def task2_hk_supplement(progress):
    """补充港股数据至近10年"""
    print("\n" + "=" * 70)
    print("任务2a: 港股数据补充至近10年 (yfinance)")
    print("=" * 70)
    ...
    for i, csv_path in enumerate(csv_files):
        ...
        hk_code = csv_path.stem  # e.g., hk00001
        print(f"  [{i+1}/{len(csv_files)}] {hk_code} (现有从{first_date}开始)")

        yf_df = fetch_hk_with_yfinance(hk_code, TEN_YEAR_START, first_date)
...
def task2_us_supplement(progress):
    """补充美股数据至近10年"""
    print("\n" + "=" * 70)
    print("任务2b: 美股数据补充至近10年 (yfinance)")
    print("=" * 70)
...
# ============================================================
# 任务3: 商品/大类资产数据 (yfinance)
# ============================================================

COMMODITY_CONFIGS = [
    # (task_key, yfinance代码, 输出文件名, 描述)
