...
# 标题：七星ETF轮动超级增强策略QMT版V3（无过滤精简版）
# 作者：任侠
# 修改：blakehao — 基于本地回测审计，关闭冗余过滤器

"""
七星ETF轮动超级增强策略 — 无过滤精简版

基于V3原版，经本地回测验证后做如下优化：
- 关闭盈利保护 (enable_profit_protection=False)：回测显示盈利保护过早截断趋势
- 关闭成交量过滤 (enable_volume_check=False)：回测显示成交量过滤误判强弱
- 关闭短期动量过滤 (use_short_momentum_filter=False)：回测显示短期动量增加噪声
- 保留溢价率过滤 (enable_premium_filter=True)：防高溢价买入，有正向贡献
- 保留行情判断 (enable_regime_switch=True)：走弱期回避A股
- 保留日内回撤保护 (enable_intraday_drawdown=True)

回测结果对比（QMT 51只ETF池, 2025.1.2-2026.5.30, 初始1万）：
  原版(全过滤): 年化332.71% 总收益+443.62% 回撤21.88% 最终54362
  无过滤版:     年化435.62% 总收益+580.83% 回撤17.29% 最终68083
  -> 年化+103%, 回撤-4.6%, 交易次数-30%

【变更记录】
- 无过滤版 [2026/06/02]：关闭盈利保护/成交量/短期动量三过滤器
- V3 [2026/05/30]
  - 修复「同日先卖后买同一标的」的 bug：
...
def initialize(context):
    ...
    # ---------- 盈利保护参数 ----------
    g.enable_profit_protection = False                     # 盈利保护开关（无过滤版: 关闭）
    ...
    # ---------- 成交量过滤 ----------
    g.enable_volume_check = False                           # 成交量过滤（无过滤版: 关闭）
    g.volume_lookback = 5
    g.volume_threshold = 3.72
    g.volume_return_limit = 1          # 年化收益>100%时启用放量过滤

    # ---------- 短期动量过滤 ----------
    g.use_short_momentum_filter = False                     # 短期动量过滤（无过滤版: 关闭）
