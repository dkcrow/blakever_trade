# 克隆自聚宽文章：https://www.joinquant.com/post/77921
# 标题：五福52V2-多ETF-2020年至今收益34倍回撤16%
# 作者：rbq2025

# 克隆自聚宽文章：https://www.joinquant.com/post/76292
# 标题：五福 V7 研究尝试｜五年半 263 倍 年化186%
# 作者：索拉尔
...
# STRATEGY_NAME=五福v1.1-择时执行-H72-H78a-merge-B
...
#行业ETF：
    ...
    # 持仓数量可按大A市场状态分别配置。
    # 正常期使用 normal_holdings_num，走弱期使用 weak_holdings_num。
    g.normal_holdings_num = 3
    g.weak_holdings_num = 2
    ...
    # 尾盘(14:55)强制买入开关：True=对剩余待买ETF无条件强制买入（原行为）；
    # False=只做最后一次趋势复检，仍不达标的直接放弃，不强制买入
    g.enable_force_buy = False
    ...
    # ==================== 相关性过滤(守卫A) ====================
    # 默认关闭，保持现有选股结果不变（需要时手动开启）
    g.enable_corr_filter = True
    g.corr_use_raw_pearson = True     # True=原始皮尔逊相关；False=修正相关性P_adj
    g.corr_threshold = 0.8            # ≥此值视为高相关，跳过该候选
    g.corr_lookback_days = 60         # 相关性回看交易日
    g.log_corr_detail = True          # 打印相关性计算与过滤明细
    g._adj_corr_cache_date = None
    g._adj_corr_cache_key = None
    g._adj_corr_cache_matrix = None

    # ==================== ETF仓位管理 ====================
    # 默认关闭=沿用原有等额现金分配
    g.enable_position_mgmt = True
    g.position_weight_metric = 'score'  # 'score'综合得分 / 'long'长期动量
    g.position_weight_power = 1.5        # 权重幂指数
    g.max_single_position = 0.5          # 单只持仓上限（占总资产比例，选出多只时生效）
    g.single_etf_max_position = 0.9      # 仅选出1只ETF时的持仓上限（占总资产比例）
    g.log_position_detail = True         # 打印仓位分配明细
    g.target_position_values = {}

    # ==================== 主线放宽 + 防御篮子 ====================
    # 【主线放宽】放宽 B 型主线的 R²/放量/连涨/增长门槛
    g.enable_p6_mainline_relax = True
    g.p6_mainline_min_r2_avg = 0.85       # 0.90 → 0.85
    g.p6_mainline_min_score_growth = 1.3  # 2.0 → 1.3
    g.p6_mainline_min_volume_avg = 1.5    # 1.8 → 1.5
    g.p6_mainline_min_score_up_days = 3   # 4 → 3
    # 【防御性篮子】无候选时改持多资产等权避险篮子（而非单一货币ETF）
    g.enable_p9_defensive_basket = True
    g.defensive_basket = ['518880.XSHG', '511880.XSHG']  # 黄金 + 货币，可自定义
    g.defensive_basket_active = False    # ← 运行态变量
    # ------------------------------------------------------------------
    # 统一应用（放在此处，确保覆盖上方已设的基准参数）
    if g.enable_p6_mainline_relax:
        g.mainline_min_r2_avg = g.p6_mainline_min_r2_avg
        g.mainline_min_score_growth = g.p6_mainline_min_score_growth
        g.mainline_min_volume_avg = g.p6_mainline_min_volume_avg
        g.mainline_min_score_up_days = g.p6_mainline_min_score_up_days
    log.info("[主线/防御] 主线放宽=%s | 防御篮子=%s"
             % (g.enable_p6_mainline_relax, g.enable_p9_defensive_basket))
    # ==================================================================

    # ==================================================================
    # ============ P0/P1/P2 优化模块（开关集中管理，默认全关）============
    # 逐个打开做单变量 A/B 测试；ON=True 启用。参数含义见每行注释。
    # ---------------------------- P0 ----------------------------------
    # A2 绝对动量闸：候选自身近 N 日收益必须 > 阈值，否则不进候选（防"下跌中的相对强者"）
    g.enable_abs_momentum = True
    g.abs_momentum_lookback = 20
    g.abs_momentum_min_ret = 0.0
    # ---------------------------- P1 ----------------------------------
    # B2 ATR 移动止损：分钟级跟踪止损（峰值回落 > atr_mult×ATR 即离场）
    g.enable_atr_stop = True
    g.atr_lookback = 25
    g.atr_mult = 3.0
    # -------------------------- 运行态（勿改）---------------------------
    g.pos_peak = {}              # 各持仓峰值价，供 B2 ATR 移动止损使用
    g._atr_cache = {}
    g._atr_cache_date = None
    log.info(
        "[P012] abs_mom=%s | atr_stop=%s"
        % (g.enable_abs_momentum, g.enable_atr_stop))
    # ==================================================================

    # ==================================================================
    # ---- T2b 止损当日禁买：盘中止损后当日禁止买回同标的
    g.enable_t2_stop_no_rebuy = True
    g.stop_loss_blacklist_today = set()
    # ==================================================================
    ...
    run_daily(afternoon_routine, time='13:18')
    ...
    run_daily(minute_level_atr_stop, time='every_bar')       # B2 ATR 移动止损
...
=== 相关性过滤(守卫A) ===
- 启用: {'是' if getattr(g, 'enable_corr_filter', False) else '否'} | 相关性口径: {'原始皮尔逊' if g.corr_use_raw_pearson else '修正相关性P_adj'}
- 高相关阈值: ≥{g.corr_threshold} 跳过候选 | 回看: {g.corr_lookback_days}个交易日
- 明细日志: {'开启' if g.log_corr_detail else '关闭'}
=== ETF仓位管理 ===
- 启用: {'是' if getattr(g, 'enable_position_mgmt', False) else '否'} | 权重口径: {g.position_weight_metric} | 幂指数: {g.position_weight_power}
- 单只持仓上限: {g.max_single_position:.0%} (仅选出1只时: {g.single_etf_max_position:.0%}) | 明细日志: {'开启' if g.log_position_detail else '关闭'}
=== 其他配置 ===
- 正常期持仓数量: {g.normal_holdings_num}只
- 走弱期持仓数量: {g.weak_holdings_num}只
...
def reset_daily_flags(context):
    ...
    g.yesterday_close_cache = {}
    g.pending_buy_etfs = []
    g.defensive_basket_active = False
    g.stop_loss_blacklist_today = set()
    log.info("🔄 收盘缓存重置完成")
...
def calculate_all_metrics_for_etf(etf, etf_name, hist_closes, hist_volumes, current_price, today_vol, context):
    try:
        ...
        # A2 绝对动量闸
        passed_abs_mom = True
        if getattr(g, 'enable_abs_momentum', False):
            _alb = int(g.abs_momentum_lookback)
            if len(price_series) >= _alb + 1 and price_series[-_alb - 1] > 0:
                passed_abs_mom = (price_series[-1] / price_series[-_alb - 1] - 1) > g.abs_momentum_min_ret
            else:
                passed_abs_mom = False
...
def apply_filters(metrics_list):
    ...
    if g.is_a_share_weak:
        steps = [
            ...
            ('绝对动量', lambda m: m.get('passed_abs_mom', True), getattr(g, 'enable_abs_momentum', False)),
...
def get_target_holdings_num():
    """根据大A当前强弱状态返回本次调仓的目标持仓数量。"""
    return g.weak_holdings_num if g.is_a_share_weak else g.normal_holdings_num


def get_final_ranked_etfs(context):
    all_metrics = []
    target_holdings_num = get_target_holdings_num()
    ...
    if len(top_10) >= target_holdings_num:
        reference_score = top_10[target_holdings_num - 1].get(score_key, float('-inf'))
        ratio = g.score_threshold_ratio if not g.is_a_share_weak else 1.0
        score_threshold = reference_score * ratio
        log_buffer.append("")
        log_buffer.append(f">>> 第三步：选取动量得分≥第{target_holdings_num}名({top_10[target_holdings_num - 1]['etf_name']})得分{reference_score:.4f}×{ratio}={score_threshold:.4f}的ETF <<<")
        candidate_pool = [item for item in top_10 if item.get(score_key, float('-inf')) >= score_threshold]
    else:
        log_buffer.append("")
        log_buffer.append(f">>> 第三步：前10名不足{target_holdings_num}只，全部作为候选池 <<<")
    ...
    if len(retained) >= target_holdings_num:
        retained_sorted = sorted(retained, key=lambda x: x.get(score_key, float('-inf')), reverse=True)
        final_result = retained_sorted[:target_holdings_num]
        log_buffer.append(f"保留的持仓ETF数量({len(retained)})超过目标持仓数({target_holdings_num})，将从保留的ETF中按动量得分取前{target_holdings_num}只作为最终目标。")
    else:
        need = target_holdings_num - len(retained)
        remaining_pool = [item for item in candidate_pool if item['etf'] not in {r['etf'] for r in retained}]
        if getattr(g, 'enable_corr_filter', False) and target_holdings_num > 1:
            additional = apply_correlation_guard(context, remaining_pool, retained, need)
        else:
...
def execute_sell_trades(context):
    ...
    g.defensive_basket_active = False
    if ranked_etfs:
        for metrics in ranked_etfs[:target_holdings_num]:
            target_etfs.append(metrics['etf'])
            log.info(f"确定最终目标: {metrics['etf']} {metrics['etf_name']}")
    else:
        if getattr(g, 'enable_p9_defensive_basket', False):
            # 【方案9】无候选 → 多资产等权避险篮子（剔除停牌/涨跌停不可交易者）
            cur = get_current_data()
            basket = []
            for code in getattr(g, 'defensive_basket', []):
                d = cur[code]
                if d.paused or d.last_price >= d.high_limit or d.last_price <= d.low_limit:
                    continue
                basket.append(code)
            if basket:
                target_etfs = basket
                g.defensive_basket_active = True
                log.info(f"🛡️ 确定最终目标(防御篮子): {[f'{c} {get_security_name(c)}' for c in basket]}")
            elif check_defensive_etf_available(context):
                target_etfs = [g.defensive_etf]
                log.info(f"🛡️ 确定最终目标(防御模式): {g.defensive_etf} {get_security_name(g.defensive_etf)}")
            else:
                log.info("💤 无最终目标(空仓模式)")
                target_etfs = []
        elif check_defensive_etf_available(context):
...
def execute_buy_trades(context):
    ...
    target_holdings_num = get_target_holdings_num()
    # 【方案9】防御篮子模式下，目标持仓数按篮子大小放开
    if getattr(g, 'defensive_basket_active', False):
        target_holdings_num = max(target_holdings_num, len(target_etfs))
    ...
    # T2b: 当日已止损的标的禁止买回
    if getattr(g, 'enable_t2_stop_no_rebuy', False):
        _bl = getattr(g, 'stop_loss_blacklist_today', set())
        _skipped = [e for e in etfs_to_buy if e in _bl]
        if _skipped:
            log.info(f"[T2b] 当日止损禁买，跳过: {_skipped}")
            etfs_to_buy = [e for e in etfs_to_buy if e not in _bl]
    actual_holding_count = len(current_positions)
    max_buy_count = max(0, target_holdings_num - actual_holding_count)
    num_etfs_to_buy = min(len(etfs_to_buy), max_buy_count)
    
    if num_etfs_to_buy <= 0:
        log.info(f"当前实际持仓数量({actual_holding_count})已达到或超过目标({target_holdings_num})，无需买入")
        log.info("========== 买入操作完成 ==========")
        return
    
    etfs_to_buy = etfs_to_buy[:num_etfs_to_buy]
    log.info(f"当前实际持仓: {actual_holding_count}只, 目标持仓: {target_holdings_num}只, 本次计划买入: {num_etfs_to_buy}只")

    g.pending_buy_etfs = list(etfs_to_buy)
    _tail_desc = "14:55强制买入" if getattr(g, 'enable_force_buy', True) else "14:55最后复检(不强制买入)"
    log.info(f"计划买入ETF: {g.pending_buy_etfs}，先进行13:10趋势判断；若不满足，将在13:40/14:10/14:40复检，{_tail_desc}")
...
def execute_buy_with_trend(context, force=False):
    ...
    # T2b: 复检期间被止损的标的，从待买列表移除
    if getattr(g, 'enable_t2_stop_no_rebuy', False):
        _bl = getattr(g, 'stop_loss_blacklist_today', set())
        _dropped = [e for e in g.pending_buy_etfs if e in _bl]
        if _dropped:
            log.info(f"[T2b] 当日止损禁买，移出待买: {_dropped}")
            g.pending_buy_etfs = [e for e in g.pending_buy_etfs if e not in _bl]
    ...
    if getattr(g, 'enable_position_mgmt', False):
        g.target_position_values = compute_target_values(context, g.target_etfs_list)
    for i, etf in enumerate(etfs_to_buy_now):
        ...
        # 仓位管理开启时：改用基于总资产的目标市值（smart_order_target_value 按目标市值下单）
        if getattr(g, 'enable_position_mgmt', False):
            target_value_for_this_etf = g.target_position_values.get(etf, target_value_for_this_etf)
...
def force_buy_pending(context):
    """14:55 尾盘处理剩余待买ETF。
    g.enable_force_buy=True: 强制买入所有待买ETF（原行为），避免因择时过严导致整段行情空仓。
    g.enable_force_buy=False: 只做最后一次趋势复检，仍不达标的直接放弃，当日不再买入。"""
    if not g.pending_buy_etfs:
        return
    log.info("★" * 80)
    if getattr(g, 'enable_force_buy', True):
        log.info("▶️ 【14:55强制买入】强制买入所有待买ETF...")
        execute_buy_with_trend(context, force=True)
        log.info("⏸️ 【14:55强制买入】执行完毕！")
    else:
        log.info("▶️ 【14:55尾盘复检】强制买入已关闭，做最后一次趋势复检...")
        execute_buy_with_trend(context, force=False)
        if g.pending_buy_etfs:
            log.info(f"🚫 【14:55尾盘复检】以下ETF趋势仍未确认，放弃买入: {g.pending_buy_etfs}")
            g.pending_buy_etfs = []
        log.info("⏸️ 【14:55尾盘复检】执行完毕！")
...
def _fetch_close_panel_for_corr(codes, lookback, end_date):
    ...
    try:
        ...
        if getattr(g, 'log_corr_detail', False):
            log.warning(f"【相关性】取价失败: {e}")
        return pd.DataFrame()


def compute_adjusted_corr_matrix(price_df, use_raw_pearson=False):
    """
    相关性矩阵。
    - use_raw_pearson=True（守卫A）：原始皮尔逊相关（对数收益率相关系数），
      同板块标的即使涨幅分化也保持高相关，能稳健拦截同类重复买入。
    - use_raw_pearson=False：修正相关性 P_adj：
...
def get_adj_corr_matrix_cached(context, codes):
    """同日同代码集同模式复用相关性矩阵。"""
    use_raw = bool(getattr(g, 'corr_use_raw_pearson', False))
    codes_key = (tuple(sorted(set(codes))), use_raw)
    today = context.current_dt.date()
    if (getattr(g, '_adj_corr_cache_date', None) == today
            and getattr(g, '_adj_corr_cache_key', None) == codes_key
            and getattr(g, '_adj_corr_cache_matrix', None) is not None):
        return g._adj_corr_cache_matrix
    lookback = int(getattr(g, 'corr_lookback_days', 60))
    price_df = _fetch_close_panel_for_corr(codes_key[0], lookback, context.previous_date)
    adj = compute_adjusted_corr_matrix(price_df, use_raw_pearson=use_raw)
    g._adj_corr_cache_date = today
    g._adj_corr_cache_key = codes_key
    g._adj_corr_cache_matrix = adj
    if getattr(g, 'log_corr_detail', False) and not adj.empty:
        mode = '原始皮尔逊' if use_raw else '修正P_adj'
        log.info(f"🔗【相关性】矩阵已更新({mode}): {adj.shape[0]}×{adj.shape[1]} 回看{lookback}日 截止{context.previous_date}")
    return adj
...
def apply_correlation_guard(context, ordered_metrics, already_selected, need):
    """
    从 ordered_metrics（按得分排序的候选 metrics）中依次挑选 need 只，
    跳过与 already_selected(已保留) 及本轮已挑中标的相关性 ≥ g.corr_threshold 的候选。
    返回被选中的 metrics 列表（顺序即挑选顺序）。
    """
    selected = []
    if need <= 0 or not ordered_metrics:
        return selected
    already_codes = [m['etf'] for m in already_selected]
    cand_codes = [m['etf'] for m in ordered_metrics]
    all_codes = list(dict.fromkeys(already_codes + cand_codes))
    adj_corr = get_adj_corr_matrix_cached(context, all_codes)
    chosen_codes = list(already_codes)
    for m in ordered_metrics:
        if len(selected) >= need:
            break
        etf = m['etf']
        name = m.get('etf_name') or get_security_name(etf)
        max_corr = None
        conflict = None
        if adj_corr is not None and chosen_codes:
            for held in chosen_codes:
                c = get_pair_adjusted_corr(etf, held, adj_corr)
                if c is None:
                    continue
                if max_corr is None or c > max_corr:
                    max_corr = c
                    conflict = held
        if max_corr is not None and max_corr >= g.corr_threshold:
            if getattr(g, 'log_corr_detail', False):
                log.info(f"🔗 跳过 {etf} {name}：与已选 {conflict} {get_security_name(conflict)} 相关性{max_corr:.3f} ≥ {g.corr_threshold}")
            continue
        selected.append(m)
        chosen_codes.append(etf)
    return selected
...
def _apply_position_cap(weights, cap):
    """
    将各标的权重限制在 cap 以内，超出部分按比例注水给未触顶标的。
    若 cap × 标的数 < 1（无法满仓），剩余权重留作现金。
    """
    if not weights:
        return {}
    ...
    uncapped = set(etfs)
    for _ in range(n + 1):
        over = [e for e in uncapped if w[e] > cap + 1e-12]
        if not over:
            break
        excess = float(np.sum([w[e] - cap for e in over]))
        for e in over:
            w[e] = cap
            uncapped.discard(e)
        if not uncapped:
            break
        sub = float(np.sum([w[e] for e in uncapped]))
        if sub <= 0:
            break
        for e in uncapped:
            w[e] += excess * (w[e] / sub)
...
def compute_target_values(context, target_etfs):
    """
    计算每只目标ETF的目标持仓金额。
    - 关闭仓位管理：等权分配（total_value / n）。
    - 开启仓位管理：按所选动量指标的比值分配；单只上限 max_single_position，
      当仅选出1只ETF时改用 single_etf_max_position（默认0.8）。
    """
    total_val = context.portfolio.total_value
    n = len(target_etfs)
    if n == 0:
        return {}
    if not getattr(g, 'enable_position_mgmt', False):
        per = total_val / n
        return {etf: per for etf in target_etfs}

    ranked = getattr(g, 'ranked_etfs_result', []) or []
    metric_map = {m['etf']: m for m in ranked}
    key = {'score': 'momentum_score', 'long': 'annualized_returns'}.get(
        getattr(g, 'position_weight_metric', 'score'), 'momentum_score')
    power = float(getattr(g, 'position_weight_power', 1.0))

    raw = {}
    for etf in target_etfs:
        val = metric_map.get(etf, {}).get(key, 0.0)
        try:
            val = max(float(val), 0.0)  # 负动量按0处理，不参与加权
        except (TypeError, ValueError):
            val = 0.0
        w_raw = val ** power if val > 0 else 0.0
        raw[etf] = w_raw

    if n == 1:
        cap = float(getattr(g, 'single_etf_max_position', 0.8))
    else:
        cap = float(getattr(g, 'max_single_position', 0.5))

    total_raw = float(np.sum(list(raw.values())))
    if total_raw <= 0:
        weights = {etf: 1.0 / n for etf in target_etfs}
        basis = '动量非正→等权兜底'
    else:
        weights = {etf: raw[etf] / total_raw for etf in target_etfs}
        basis = f'{key}比值^{power:g}'
    weights = _apply_position_cap(weights, cap)
    target_values = {etf: total_val * w for etf, w in weights.items()}

    if getattr(g, 'log_position_detail', True):
        invested = float(np.sum(list(weights.values())))
        log.info(f"⚖️ 仓位分配（依据{basis}，单只上限{cap*100:.0f}%）：")
        for etf in target_etfs:
            mval = metric_map.get(etf, {}).get(key, 0.0)
            mval = mval if isinstance(mval, (int, float)) else 0.0
            log.info(f"    {etf} {get_security_name(etf)} 权重{weights[etf]*100:5.1f}% 目标{target_values[etf]:,.0f}元 ({key}={mval*100:.1f}%)")
        if invested < 0.999:
            log.info(f"    （受上限约束，投资比例{invested*100:.1f}%，剩余{(1-invested)*100:.1f}%留作现金）")
...
# ==================== 🧩 P0/P1/P2 优化：新增函数 ====================
def minute_level_atr_stop(context):
    """B2 ATR 移动止损：现价从持仓峰值回落超过 atr_mult×ATR(日线) 即离场。ATR 按日缓存。"""
    if not getattr(g, 'enable_atr_stop', False):
        return
    ct = context.current_dt.strftime('%H:%M')
    if not (('09:35' < ct < '11:30') or ('13:00' < ct < '14:57')):
        return
    today = context.current_dt.date()
    if getattr(g, '_atr_cache_date', None) != today:
        g._atr_cache = {}
        g._atr_cache_date = today
    cur = get_current_data()
    peaks = getattr(g, 'pos_peak', {}) or {}
    # 清理已清仓标的的峰值记录
    peaks = {s: v for s, v in peaks.items()
             if s in context.portfolio.positions and context.portfolio.positions[s].total_amount > 0}
    lb = int(getattr(g, 'atr_lookback', 20))
    mult = float(getattr(g, 'atr_mult', 3.0))
    for sec in list(context.portfolio.positions.keys()):
        pos = context.portfolio.positions[sec]
        if pos.total_amount <= 0 or pos.closeable_amount <= 0:
            continue
        px = cur[sec].last_price
        if px is None or px <= 0:
            continue
        atr = g._atr_cache.get(sec)
        if atr is None:
            try:
                df = attribute_history(sec, lb + 1, '1d', ['high', 'low', 'close'], skip_paused=False)
                if df is None or len(df) < lb + 1:
                    continue
                highs = df['high'].values; lows = df['low'].values; closes = df['close'].values
                prev_close = closes[:-1]; h = highs[1:]; l = lows[1:]
                tr = np.maximum(h - l, np.maximum(np.abs(h - prev_close), np.abs(l - prev_close)))
                atr = float(np.mean(tr[-lb:]))
                g._atr_cache[sec] = atr
            except Exception:
                continue
        if atr is None or atr <= 0:
            continue
        peak = max(peaks.get(sec, pos.avg_cost), px)
        peaks[sec] = peak
        if px < peak - mult * atr:
            drop = (px / peak - 1) * 100
            log.info(f"🚨【ATR移动止损】{sec} {get_security_name(sec)} 现价{px:.3f} 距峰值{peak:.3f} 回落{drop:.2f}% "
                     f"(>{mult}×ATR={mult*atr:.3f}) → 离场")
            smart_order_target_value(sec, 0, context)
            if getattr(g, 'enable_t2_stop_no_rebuy', False):
                g.stop_loss_blacklist_today.add(sec)
                log.info(f"[T2b] {sec} 加入当日禁买名单")
    g.pos_peak = peaks
