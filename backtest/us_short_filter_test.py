#!/usr/bin/env python3
"""七星美股版 短期动量过滤回测: short_score < 0 → 过滤不买入"""
import sys, math, warnings
from pathlib import Path
import numpy as np, pandas as pd
warnings.filterwarnings('ignore')
sys.path.insert(0, str(Path(__file__).parent.parent))

PROJECT = Path(__file__).parent.parent
DATA_DIR = PROJECT / 'data' / 'storage' / 'stock_data' / 'us'

POOL = ['NVDA','AMD','MU','LRCX','LITE','NFLX','GOOGL','NOW','ORCL','SNPS',
        'EOG','NEM','CAT','GE','AMT','PANW','ZS','NET','IONQ','RKLB',
        'SPCX','COHR','HOOD','WDC','ARM','STX']
HN = 7; COMM = 0.005; SLIP = 0.0005; SCORE_THR = 0.5
MIN_HISTORY_DAYS = 126

def calc_score(close_full, lookback=25):
    recent = close_full[-(lookback+1):]
    y = np.log(np.maximum(recent, 1e-10))
    x = np.arange(len(y)); w = np.linspace(1, 2, len(y))
    slope, intercept = np.polyfit(x, y, 1, w=w)
    ann = math.exp(slope * 250) - 1
    ssr = np.sum(w * (y - (slope*x + intercept))**2)
    sst = np.sum(w * (y - np.mean(y))**2)
    r2 = 1 - ssr/sst if sst>0 else 0
    return ann * r2

def get_ranked(all_data, prices, date, filter_short=False):
    ranked = []
    for code, df in all_data.items():
        if code not in prices: continue
        mask = df.index < pd.Timestamp(date); hist = df[mask]
        if len(hist) < MIN_HISTORY_DAYS: continue
        cp = prices[code]
        if cp <= 0: continue
        long_score = calc_score(hist['close'].values, 25)
        short_score = calc_score(hist['close'].values, 10) if len(hist) >= 11 else 0
        chg_pct = 0.0
        if len(hist) >= 1:
            prev = hist['close'].iloc[-1]
            if prev > 0: chg_pct = (cp - prev) / prev * 100
        filtered = filter_short and short_score < 0
        ranked.append({'code':code,'score':long_score,'short_score':short_score,'price':cp,'chg_pct':chg_pct,'filtered':filtered})
    ranked.sort(key=lambda x: x['score'], reverse=True)
    return ranked

def run(start, end, filter_short):
    # Load data
    all_data = {}
    for sym in POOL:
        fp = DATA_DIR / f'{sym}.csv'
        if not fp.exists(): continue
        df = pd.read_csv(fp)
        df.columns = [c.lower().strip() for c in df.columns]
        dc = [c for c in df.columns if c.lower()=='date'][0]
        df = df.rename(columns={dc: 'date'})
        df['date'] = pd.to_datetime(df['date']); df = df.set_index('date').sort_index()
        m = (df.index >= start) & (df.index <= end)
        df = df[m]
        if len(df) >= 25: all_data[sym] = df

    td = sorted(set().union(*[set(df.index) for df in all_data.values()]))
    td = [d for d in td if start <= d.strftime('%Y-%m-%d') <= end]

    cash = 1_000_000; pos = {}
    daily = []

    for date in td:
        ds = date.strftime('%Y-%m-%d'); tds = pd.Timestamp(date)
        prices = {}
        for code, df in all_data.items():
            m = df.index == date
            if m.any(): prices[code] = float(df.loc[date, 'close'])
        if len(prices) < HN: continue

        ranked = get_ranked(all_data, prices, date, filter_short)
        targets = [r for r in ranked if r['score'] >= SCORE_THR and not r.get('filtered', False)][:HN]
        tc = set(r['code'] for r in targets)

        # Sell
        for code in list(pos.keys()):
            if code in tc: continue
            p = prices.get(code)
            if not p:
                # stale position: use last known
                p = pos[code]['cp']
            sp = p * (1 - SLIP)
            tv2 = pos[code]['shares'] * sp
            comm_cost = max(pos[code]['shares'] * COMM, 1)
            cash += tv2 - comm_cost
            del pos[code]

        # Buy
        new = [r for r in targets if r['code'] not in pos]
        if new:
            avail = cash * 0.95; per = avail / len(new)
            for r in new:
                bp = r['price'] * (1 + SLIP); sh = int(per / bp)
                if sh < 1: continue
                cost = sh * bp + max(sh * COMM, 1)
                cash -= cost
                pos[r['code']] = {'shares': sh, 'cp': bp}

        # Daily NAV
        tv = cash
        for c, p2 in pos.items():
            tv += p2['shares'] * prices.get(c, p2['cp'])
        daily.append(tv)

    if not daily: return {}
    tv = daily[-1]
    tr = (tv / 1_000_000 - 1) * 100
    days = len(daily)
    cagr = ((tv / 1_000_000) ** (252 / max(days, 1)) - 1) * 100
    peak = 1_000_000; mdd = 0
    for v in daily:
        if v > peak: peak = v
        dd = (v - peak) / peak * 100
        if dd < mdd: mdd = dd
    rets = np.diff(daily) / daily[:-1]
    sharpe = np.mean(rets) / np.std(rets) * np.sqrt(252) if len(rets)>0 and np.std(rets)>0 else 0
    return {'tr': round(tr,1), 'cagr': round(cagr,1), 'mdd': round(mdd,1), 'sh': round(sharpe,2), 'tv': tv}


periods = [('1年','2025-07-02','2026-07-02'),('3年','2023-07-02','2026-07-02'),('5年','2021-07-02','2026-07-02')]

for pn, start, end in periods:
    print(f'\n[{pn}] {start}~{end}')
    r_off = run(start, end, False)
    r_on = run(start, end, True)
    if r_off and r_on:
        star = ' ★' if r_on['tr'] > r_off['tr'] and abs(r_on['mdd']) < abs(r_off['mdd']) else ''
        print(f'  关闭短期过滤: +{r_off["tr"]}% 年化{r_off["cagr"]}% 回撤{r_off["mdd"]}% 夏普{r_off["sh"]}')
        print(f'  启用短期过滤: +{r_on["tr"]}% 年化{r_on["cagr"]}% 回撤{r_on["mdd"]}% 夏普{r_on["sh"]}{star}')

print('\n★ = 收益>关闭 且 回撤<关闭')
print('完成!')
