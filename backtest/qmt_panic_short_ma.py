#!/usr/bin/env python3
"""七星QMT 恐慌期空仓 — 短均线寻优 (80%阈值, MA 5/10/15日 vs 关闭) 验证"是否越短越好"
3年 + 5年, 标注触发次数/空仓天数/平均段长/最长段。唯一变量=panic_ma_lookback。
"""
...
PERIODS = [('3年', '2023-06-21', '2026-06-21'),
           ('5年', '2021-06-21', '2026-06-21')]

for label, s, e in PERIODS:
    rows = [(name, run_one(s, e, en, lb)) for name, en, lb in CONFIGS]
    base = rows[0][1]
    print(f"\n{'='*120}")
    print(f"  七星QMT 恐慌期短均线寻优 — {label} ({s} ~ {e}, {base['days']}交易日) [阈值80%, 持仓1只]")
    print('='*120)
